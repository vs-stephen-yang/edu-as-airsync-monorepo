...
@property(atomic) CVPixelBufferRef displayBuffer;
@property(nonatomic) CVPixelBufferRef workingBuffer;
@end
...
- (void)fillBufferWithColor:(CVPixelBufferRef)buffer
    ...
    if (baseAddress) {
        vImage_Buffer vBuffer = {
            .data = baseAddress, .height = height, .width = width, .rowBytes = bytesPerRow};
    }
...
- (void)updateWithRGBAData:(const uint8_t *)data
    ...
    @try {
        if (_lastWidth != width || _lastHeight != height) {
            ...
            if (_displayBuffer) {
                [self fillBufferWithColor:_displayBuffer red:0 green:0 blue:50 alpha:255];
            }
        }
