...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  if ([@"receiveStart" isEqualToString:call.method]) {
    ...
    // 檢查所有必要參數是否存在
    if (!multicastIp || !videoPortNumber || !audioPortNumber || !ssrcNumber ||
        !keyData || !saltData || !videoRocNumber || !audioRocNumber) {
      result([FlutterError
          errorWithCode:@"MISSING_ARGUMENT"
    ...
    while (temp_addr != NULL) {
      ...
      if (temp_addr->ifa_addr->sa_family == AF_INET) {
        ...
        if ((temp_addr->ifa_flags & IFF_UP) &&
          ...
          if (inet_ntop(AF_INET, &(addr_in->sin_addr), addressBuffer,
                        INET_ADDRSTRLEN)) {
            NSString *ipString = [NSString stringWithUTF8String:addressBuffer];
            [ipArray addObject:ipString];

            NSLog(@"Found IP: %@ on interface: %s", ipString,
                  temp_addr->ifa_name);
          }
        }
      }
...
- (void)setupGlobalTextureVariables {
  set_global_texture_variables((__bridge void *)_videoTexture,
                               (__bridge void *)_textureRegistry, _textureId);
}
