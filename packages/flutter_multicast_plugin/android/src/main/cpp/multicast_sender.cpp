...
Java_com_viewsonic_flutter_1multicast_1plugin_NativeBridge_startRtpStream(
    JNIEnv *env, jobject thiz, jobjectArray local_ips, jstring ip,
    jint video_port, jint audio_port, jbyteArray jKey, jbyteArray jSalt,
    jint ssrc) {
  ...
  for (const std::string &local_ip : local_ip_list) {
    ...
    // 建立 video stream
    pair.videoStream =
        pair.session->create_stream(video_port, RTP_FORMAT_H264, flags);
    if (!pair.videoStream) {
    ...
    // 建立 audio stream
    pair.audioStream =
        pair.session->create_stream(audio_port, RTP_FORMAT_OPUS, flags);
    if (!pair.audioStream) {
  ...
  ALOGI("uvgRTP streams and sessions created successfully for %zu local IPs",
        streamPairs.size());
  return JNI_TRUE;
}

extern "C" JNIEXPORT jobject JNICALL
Java_com_viewsonic_flutter_1multicast_1plugin_NativeBridge_getStreamRoc(
...
Java_com_viewsonic_flutter_1multicast_1plugin_NativeBridge_sendAudioRtpFrame(
  ...
  for (auto &pair : streamPairs) {
    if (pair.audioStream) {
      rtp_error_t ret = pair.audioStream->push_frame(
          reinterpret_cast<uint8_t *>(buf), len, RTP_NO_FLAGS);
      if (ret != RTP_OK) {
        ALOGE("Failed to send audio data via local IP: %s",
              pair.local_ip.c_str());
        continue;
      }
    }
...
extern "C" JNIEXPORT void JNICALL
Java_com_viewsonic_flutter_1multicast_1plugin_NativeBridge_stopRtpStream(
    JNIEnv *env, jobject thiz) {
  cleanupStreamPairs();
  if (ctx) {
    delete ctx;
