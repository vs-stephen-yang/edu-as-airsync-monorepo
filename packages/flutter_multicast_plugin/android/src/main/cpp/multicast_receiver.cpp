...
Java_com_viewsonic_flutter_1multicast_1plugin_NativeBridge_receiveStart(
  ...
  env->GetByteArrayRegion(j_salt, 0, 14,
  ...
  g_receiver->start(
      local_ip_list, c_ip, video_port, audio_port, key, salt, native_ssrc,
      video_roc_value, audio_roc_value,
      [](const std::vector<uint8_t> &au) { g_pipeline->push_au(au); },
